...
Fl_Menu_Item customer_menuitems[] = {
	{"&File", 0,0,0,FL_SUBMENU},
		{"&Open", FL_ALT + 'o', (Fl_Callback *) open_fileCB},
		{"&Save", FL_ALT + 's', (Fl_Callback *) save_fileCB},
		{"&Quit", FL_ALT + 'q', (Fl_Callback *) Quit},
...
int main()
{
	string sales_associate_password = "9999";
	string manager_password = "3210";
	string boss_password = "9611";
	string customer_password = "0000";
	...
	while(true)
	{
		input_password = fl_password("Enter 4-Digit System PIN: ", "");
		login->hide();
		login->clear();
...
Sales Associates:
    	Sales associates will handle the order process for the customer once the
    	customer selects a model. The Sales Associate will then provide the invoice to
    	the customer.
    	Additionally, the Sales Associates will also be able to view there sales reports.

Product Manager:
    	The Product Manager is able to create new robot parts, as well as assembling
...
Pointed-haired Boss:
		...
		buff->text(help_text.c_str());
			...
			win->show();
			return Fl::run();
		}else if(input_password == boss_password)
		{
			Fl_Window *win;
			Fl_Menu_Bar *menubar;
			fl_register_images();
			win = new Fl_Window(640,480, "Robbie Robot Shop (Boss)");
			menubar = new Fl_Menu_Bar(0,0,640,30);
			string help_text = R"(
                        		===========================
		                           		   HELP
...
Pointed-haired Boss:
		...
		win->show();
		buff->text(help_text.c_str());
			menubar->menu(boss_menuitems);
			win->end();
			win->show();
			return Fl::run();
		}else if(input_password == customer_password)
		{
			Fl_Window *win;
			Fl_Menu_Bar *menubar;
			fl_register_images();
			win = new Fl_Window(640,480, "Robbie Robot Shop (Customer)");
			menubar = new Fl_Menu_Bar(0,0,640,30);
			string help_text = R"(
                        		===========================
		                           		   HELP
...
Pointed-haired Boss:
		...
		win->show();
		buff->text(help_text.c_str());
			menubar->menu(customer_menuitems);
			win->end();
			win->show();
			return Fl::run();
		}
