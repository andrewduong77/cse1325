...
int main()
{
    ...
    for(Employee* it : staff)
    {
        if(it->get_type() == "Stock_Boy")
            ((Stock_boy*)it)->add_hours(10);
        if(it->get_type() == "Sales")
            (dynamic_cast<Sales*>(it))->add_sales(100000);
            //When casting to a class that virtually inherits the base, you have to use dynamic_cast.
        cout << it->get_name() << " " << it->pay(0) << endl;
        //cout<< it->get_type();
    }
    
    vector<Stock_boy> numbers;
   // Stock_boy temp {variables};
